...
def load_data():
    ...
    data=loadmat('train_TC.mat')
    X_train0 = data['train_TC'].transpose()
    
    data=loadmat('test_TC.mat')
    X_test = data['test_TC'].transpose()

    data=loadmat('train_labels.mat')
    y_train0 = np.squeeze(data['train_labels'])
    
    data=loadmat('test_labels.mat')
    ...
    X_train = X_train.reshape((X_train.shape[0], 3, DIM, DIM))
    X_valid = X_valid.reshape((X_valid.shape[0], 3, DIM, DIM))
    X_test = X_test.reshape((X_test.shape[0], 3, DIM, DIM))
...
def build_model(input_width, input_height, output_dim,
    ...
    class_l6 = conv(class_l5,num_filters=128,filter_size=(5, 5),nonlinearity=relu,W=ini,pad=2)
    class_l7 = dense(class_l6,num_units=3072,nonlinearity=relu,W=ini)
    class_l8 = dense(class_l7,num_units=3072,nonlinearity=relu,W=ini)
    
    l_out1 = dense(class_l8,num_units=output_dim,nonlinearity=lasagne.nonlinearities.softmax,W=ini)
    l_out2 = dense(class_l8,num_units=output_dim,nonlinearity=lasagne.nonlinearities.softmax,W=ini)
    l_out3 = dense(class_l8,num_units=output_dim,nonlinearity=lasagne.nonlinearities.softmax,W=ini)
    l_out4 = dense(class_l8,num_units=output_dim,nonlinearity=lasagne.nonlinearities.softmax,W=ini)
    l_out5 = dense(class_l8,num_units=output_dim,nonlinearity=lasagne.nonlinearities.softmax,W=ini)
