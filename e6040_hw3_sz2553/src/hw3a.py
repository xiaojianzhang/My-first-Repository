...
def noise_injection(x, var=0.001):
...
def test_noise_inject_at_input(learning_rate=0.01, w_noise=1e-6,
    ...
    # Load down-sampled dataset in raw format (np.darray, not Theano.shared)
    # train_set, valid_set, test_set format: tuple(input, target)
    # input is a np.ndarray of 2 dimensions (a matrix), where each row
    # corresponds to an example. target is a np.ndarray of 1 dimension
    # (vector) that has the same length as the number of rows in the input.

    # Load dataset in raw Format, since we need to preprocess it
    train_set, valid_set, test_set = load_data(ds_rate=5, theano_shared=False)

    # Repeat the training set 5 times
    train_set[0] = np.tile(train_set[0], [5, 1])
    train_set[1] = np.tile(train_set[1], 5)

    # TODO: injection noise to the training set
    train_set[0] = noise_injection(train_set[0])
...
def test_noise_inject_at_weight(learning_rate=0.01,
    ...
    # TODO: inject noise to the weight
...
def test_adversarial_example(learning_rate=0.01, L1_reg=0.00,
    ...
    # allocate symbolic variables for the data
    index = T.lscalar()  # index to a [mini]batch
    adv_error = T.vector()  # error of adversarial example
    ...
    ginparam = T.grad(cost, classifier.input)
    ...
    # TODO add sign
    test_adv_model = theano.function(
        inputs=[index, adv_error],
        outputs=[ginparam, classifier.p_y_given_x],
        givens={
    ...
    test_adv_example = theano.function(
        inputs=[index, adv_error],
    ...
    train_nn(train_model, validate_model, test_model,
             n_train_batches, n_valid_batches, n_test_batches,
             n_epochs, verbose, np.zeros(shape=train_set_x.get_value()[0].shape,
                                         dtype=np.float32))

    adv_grad, p_y_given_x = test_adv_model(0, np.zeros(shape=train_set_x.get_value()[0].shape,
                                                       dtype=np.float32))
    adv_p_y_given_x = test_adv_example(0, -1e-2*np.sign(adv_grad).flatten())
    
    return p_y_given_x, adv_p_y_given_x
