...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000,
             batch_size=20, n_hidden=500, n_hiddenLayers=5, verbose=False,
             datasets=None):
    """
...
def grid_search_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000,
             batch_size=20, n_hidden=500, n_hiddenLayers=5, verbose=False):
    """
    Performs grid search on various combination of parameters

    :type learning_rate: float or list of float
    :param learning_rate: learning rate used (factor for the stochastic
    gradient

    :type L1_reg: float or list of float
    :param L1_reg: L1-norm's weight when added to the cost (see
    regularization)

    :type L2_reg: float or list of float
    :param L2_reg: L2-norm's weight when added to the cost (see
    regularization)
    ...
    :type n_hiddenLayers: int or list of ints
    :param n_hiddenLayers: number of hidden layers
    ...
    learning_rate, L1_reg, L2_reg, batch_size, n_hidden, n_hiddenLayers = \
        map(lambda x: x if hasattr(x, '__iter__') else (x,),
            (learning_rate,L1_reg,L2_reg,batch_size,n_hidden,n_hiddenLayers))
    grid = product(learning_rate,L1_reg,L2_reg,batch_size,n_hidden,n_hiddenLayers)
    n_grid = numpy.prod(map(len,(learning_rate,L1_reg,L2_reg,batch_size,n_hidden,n_hiddenLayers)))

    print('... starting grid search with %d combinations of parameters\n')

    for i, (lr, l1, l2, bs, nh, nhl) in enumerate(grid):
        # print out parameters
        print('=== Parameter set: %.4d/%.4d] ===' % (i,n_grid))
        print('Learning Rate: %f\n'
              'L1 Reg: %f\nL2 Reg: %f\n'
              'Batch Size: %d\n'
              'Num. of Hidden Layer: %d\n'
              'Num. of Hidden Unit: %d' %
              (lr,l1,l2,bs,nhl,nh))
        print('---------------------------------')

        test_mlp(learning_rate=lr, L1_reg=l1, L2_reg=l1, n_epochs=n_epochs,
             batch_size=bs, n_hidden=nh, n_hiddenLayers=nhl, verbose=verbose,
             datasets=datasets)

        print('\n')

if __name__ == '__main__':
    grid_search_mlp(n_epochs=5,n_hiddenLayers=[1,2,3,4,5,6,7], n_hidden=[300])
