...
class RNNSLU2(object):
    ...
    def __init__(self, nh, nh2, nc, ne, de, cs, normal=True):
        ...
        :type nh_2: int
        :param nh_2: dimension of the second hidden layer
        ...
        self.h0 = theano.shared(name='h0',
                                value=numpy.zeros(nh,
                                dtype=theano.config.floatX))
        #new part for the second hidden layer:
        #initial second hidden layer
        self.hh0 = theano.shared(name='hh0',
                                value=numpy.zeros(nh2,
                                dtype=theano.config.floatX))
        #second hidden layer sharing weights
        self.wh2 = theano.shared(name='wh2',
                                value=0.2 * numpy.random.uniform(-1.0, 1.0,
                                (nh2, nh2))
                                .astype(theano.config.floatX))
        #second hidden layer sharing bias
        self.bh2 = theano.shared(name='bh2',
                                value=numpy.zeros(nh2,
                                dtype=theano.config.floatX))
        #first hidden layer to second hidden layer sharing weights
        self.wh1 = theano.shared(name='wh1',
                               value=0.2 * numpy.random.uniform(-1.0, 1.0,
                               (nh, nh2))
                               .astype(theano.config.floatX))
        #first hidden layer to second hidden layer sharing bias
        ...
        self.params = [self.emb, self.wx, self.wh, self.w,
                       self.bh, self.b, self.h0, self.hh0, 
                       self.wh2, self.bh2, self.wh1, self.bh1]
        ...
        def recurrence(x_t, h_tm1, h2_tm1):
            ...
            h2_t = T.nnet.sigmoid(T.dot(h_t, self.wh1) + self.bh1
        ...
        [h, h2, s], _ = theano.scan(fn=recurrence,
                                sequences=x,
                                outputs_info=[self.h0, self.hh0, None],
                                n_steps=x.shape[0])
